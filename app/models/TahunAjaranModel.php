...
class TahunAjaranModel
{
    ...
    {
        $this->db->query("SELECT id, nama, status FROM " . $this->table_name . " ORDER BY id DESC");
        return $this->db->resultSet();
    }

    public function getActive()
    {
        $this->db->query("SELECT id, nama, status FROM " . $this->table_name . " WHERE status = 'Aktif' LIMIT 1");
        return $this->db->single();
    }

    public function getById($id)
    {
        $this->db->query("SELECT id, nama, status FROM " . $this->table_name . " WHERE id = :id");
        $this->db->bind('id', $id);
        return $this->db->single();
    }
