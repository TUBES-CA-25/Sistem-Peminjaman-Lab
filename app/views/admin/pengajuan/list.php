        ...
        <tbody>
            ...
            <?php 
                ...
                $dataJSON = htmlspecialchars(json_encode([
                    ...
                    'proposal' => BASE_URL . '/uploads/' . $row['file_proposal'],
                    'status' => $row['status'],
            ...
            <tr>
                ...
                <td>
                    <div class="small">
                        <div class="mb-1"><span class="text-primary fw-bold">Mulai:</span> <?= date('d/m/Y', strtotime($row['tgl_mulai'])); ?></div>
                        <div><span class="text-danger fw-bold">Selesai:</span> <?= date('d/m/Y', strtotime($row['tgl_selesai'])); ?></div>
                    </div>
                </td>

                <td class="text-center">
                <?php if (!empty($row['file_proposal'])): ?>
                    <a href="<?= BASE_URL; ?>/uploads/<?= $row['file_proposal']; ?>" 
                    target="_blank" 
                    class="btn btn-sm btn-outline-danger shadow-sm py-1" 
                    title="Lihat / Download Proposal">
                ...
                <td class="text-center">
                    <?php 
                        ...
                        if($row['status'] == 'Disetujui') $statusClass = 'bs-success'; 
                        elseif($row['status'] == 'Ditolak') $statusClass = 'bs-danger'; 
                        elseif($row['status'] == 'Menunggu Interview') $statusClass = 'bs-info'; 
                    ?>
