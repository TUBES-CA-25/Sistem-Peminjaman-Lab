...
<div style="background: linear-gradient(135deg, #122E4F 0%, #1F45AC 100%); padding: 2.5rem; border-radius: 12px; margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 6px rgba(0,0,0,0.1); gap: 1.5rem; flex-wrap: wrap;">
  <div style="flex: 1; min-width: 250px;">
    ...
    <p style="color: rgba(255,255,255,0.9); font-size: 1rem; margin:0;">Lihat jadwal praktikum dan peminjaman semua lab</p>
  </div>
</div>
    ...
    <div class="container-fluid px-0">
        ...
        <div class="mb-4">
            <div class="p-date-picker">
                ...
                <input type="date" id="jadwalDate" value="<?= htmlspecialchars($data['selected_date']) ?>" onchange="changeJadwalDate(this.value)" style="padding: 8px 14px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 14px;" />
            </div>
        </div>
                ...
                <div class="p-slot-list">
                    ...
                    <?php foreach ($allSlots as $slot): ?>
                        ...
                        <?php elseif ($slot['type'] == 'kosong'): $k = $slot['data']; ?>
                            <div class="p-slot" style="background: #F8FAFC; border: 1px dashed #CBD5E1; color: #94A3B8; cursor: default;">
                                <span class="p-slot-label" style="display:block; margin-bottom:4px;">Kosong</span>
                                <span class="p-slot-sub" style="display:block; font-size:12px;"><?= $k['mulai'] ?>-<?= $k['selesai'] ?></span>
                            </div>
